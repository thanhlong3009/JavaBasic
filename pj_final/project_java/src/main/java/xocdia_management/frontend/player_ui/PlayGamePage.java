...
public class PlayGamePage {
    ...
    public void run(String email) {
        ...
        while (!isQuit) {
            System.out.println("---- BỘ MÔN THỂ THAO TÀI XỈU ----");
            ...
            switch (option) {
                case 1: {
                    ...
                    do {
                        try {
                            System.out.println("Nhập số tiền bạn muốn đặt cược ( 0 - " + player.getAccountBalance() + " ):");
                            amountBet = Integer.parseInt(sc.nextLine());
                        } catch (NumberFormatException e) {
                            System.out.println("Lựa chọn không hợp lệ");
                            continue;
                        }
                    } while (amountBet <= 0 || amountBet > player.getAccountBalance());
                    ...
                    int betting = playerController.betting();
